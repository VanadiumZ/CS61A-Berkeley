def shuffle(s):
    ...
    for i in range(len(s) // 2):
        array.append(firsthalf[i])
        array.append(secondhalf[i])
    return array
...
def deep_map(f, s):
    ...
    """
    "*** YOUR CODE HERE ***"
    # deep_map here is a direct function return nothing for you should not create anything new
...
def balanced(m):
    ...
    if is_mobile(m):
        leftend = end(left(m))
        rightend = end(right(m))
        left_balance = balanced(leftend)
        right_balance = balanced(rightend)
        return left_balance and right_balance and (length(left(m))*total_mass(leftend) == length(right(m))*total_mass(rightend))
