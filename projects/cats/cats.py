...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(paras):
        std_para = lower(remove_punctuation(paras)).split()
        for word in std_para:
            if word in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    # consider two pointer -- emmm it's kind of similar to the string comparing
    # importantly, put the typed length as the denominator of acc
    ...
    matched = 0
    for i in range(min(type_len, source_len)):
        if (typed_words[i] == source_words[i]):
            matched += 1
    
    return matched / type_len * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff < mindiff and diff <= limit:
            mindiff = diff
            best = word
    return best
...
def furry_fixes(typed, source, limit):
    ...
    #         return limit + 1
    # change_count += abs(len(typed) - len(source))
    # return change_count
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        # use recursion here to test all paths and use limit to prune
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[tpp[i][j]-tpp[i][j-1] for j in range(1, len(tpp[0]))] for i in range(len(tpp))]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest = [[] for _ in range(len(player_indices))]
    for i in word_indices:
        word_times = [get_time(times, j, i) for j in player_indices]
